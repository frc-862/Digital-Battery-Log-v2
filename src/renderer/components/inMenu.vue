<template>
    <div class="container">
        ...
        <div class="in-menu-container" v-show="submitted == false">
            <div class="info">
            ...
            <div class="submit-warning" v-show="submitWarn == true">
                <i class="fa-solid fa-triangle-exclamation fa-2x"></i>
                <p class="submit-warning-message">Warning: Battery {{ battery.slice(0,2) }}.{{ battery.slice(2,4) }} is already signed in!</p>
        ...
        <div class="submitted" v-show="submitted == true">
            <p class="signed-in-message">{{ signInMessage }}</p>
        </div>
...
let signInMessage: Ref<string> = ref("");
...
async function submitButton() {
    ...
    if (isOut == false) {
        ...
        submitWarnMessage.value = `Warning: Battery ${battery.value} is already signed in! Are you sure you want to proceed?`;
        return;
    } else {
        submit();
    }
...
async function submit() {
    ...
    const success: boolean = await signIn(battery.value, soc.value, rint.value);
    if (success) {
        signInMessage.value = "Successfully signed In!";
    } else {
        signInMessage.value = "Failed to sign In!";
    }
    setTimeout(() => {
        navigateTo("/")
    }, 3000)
